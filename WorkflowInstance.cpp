...
WorkflowInstance::WorkflowInstance(const char *workflow_name,WorkflowParameters *parameters, unsigned int workflow_schedule_id,const char *workflow_host, const char *workflow_user)
{
	...
	try
	{
		...
		if(savepoint_level>=2)
		{
			...
			db.QueryPrintf("INSERT INTO t_workflow_instance(workflow_id,workflow_schedule_id,workflow_instance_host,workflow_instance_status,workflow_instance_start) VALUES(%i,%i,%s,'EXECUTING',NOW())",&workflow_id,workflow_schedule_id?&workflow_schedule_id:0,workflow_host);
...
pid_t WorkflowInstance::TaskExecute(DOMNode *task_node,pid_t tid,bool *workflow_terminated)
{
	...
	if(pid==0)
	{
		...
		if(fno==-1)
		{
			...
			// TODO: This will create a zombie process as no exit message will be sent
			exit(-1);
		}
