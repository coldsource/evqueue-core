...
// evQueue core
static map<string, string> evqueue_query = {
	{"t_queue", "INSERT INTO t_queue(queue_name,queue_concurrency,queue_scheduler) VALUES('default',1,'default');"},
	{"t_user", "INSERT INTO t_user(user_login,user_password,user_profile,user_preferences) VALUES('admin',SHA1('admin'),'ADMIN','');"}
};
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='v3.3'; \
"},
{"t_schedule",
...
static auto init = DBConfig::GetInstance()->RegisterTables("evqueue", evqueue_tables);
static auto initq = DBConfig::GetInstance()->RegisterTablesInit("evqueue", evqueue_query);
